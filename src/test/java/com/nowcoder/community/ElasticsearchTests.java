...
//    }
    ...
    public void testSearchByRepository(){
        SearchQuery searchQuery = new NativeSearchQueryBuilder()
                .withQuery(QueryBuilders.multiMatchQuery("互联网寒冬", "title", "content"))// 查询，搜索条件
                .withSort(SortBuilders.fieldSort("type").order(SortOrder.DESC)) // 以type为首要排序依据，倒序(置顶，加精)
                .withSort(SortBuilders.fieldSort("score").order(SortOrder.DESC))
                .withSort(SortBuilders.fieldSort("createTime").order(SortOrder.DESC))
                .withPageable(PageRequest.of(0,10))  // 从0页开始（也就是第一页），每页10条数据
                .withHighlightFields(  // 高亮显示字段设置， Tag : <em>高亮
                        new HighlightBuilder.Field("title").preTags("<em>").postTags("</em>"),
                        new HighlightBuilder.Field("content").preTags("<em>").postTags("</em>")
                ).build(); // .build()方法呼叫后，返回SearchQuery对象的实现类

        // 问题 ： 返回的discussPost并没有添加<em>
        // 因为Repository的底层会返回两份数据，一份原始数据，一份新增内容，所以需要手动添加
        // elasticTemplate.queryForPage(searchQuery, class,SearchResultMapper)
        Page<DiscussPost> page = discussRepository.search(searchQuery);
        System.out.println(page.getTotalElements());
        System.out.println(page.getTotalPages());
        System.out.println(page.getNumber()); // 当前在多少页
        System.out.println(page.getSize()); // 每页最多显示几条数据
