...
public class EventConsumer implements CommunityConstant {
    ...
    public void handleShareMessage(ConsumerRecord record){
        ...
        try {
            Runtime.getRuntime().exec(cmd); //该行代码执行完成需要时间很多，但是程序会往下走，如果后面的程序需要用到图片，需要等待图片生成完成
        ...
        // 启动定时器，监视该图片，一旦图片生成了，就上传至服务器，否则超过30s, 放弃并且宣告失败
    ...
    class UploadTask implements Runnable{
        ...
        // 启动任务的返回值taskScheduler.scheduleAtFixedRate（），它可以用于停止任务
